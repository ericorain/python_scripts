...
async def ping_coroutine(cmd, ip):
    ...
    #
    # Output:
    # - nbr_host_found (integer, global variable): this number is incremented if a ping answer has been received
    # - list_of_hosts_found (list of string, global variable): the IP address - which is "ip" input parameter - is added to this list
...
class Networkscan:
    ...
    def __init__(self, ip_and_prefix):
        ...
        # - one_ping_param (string): The parameter for Windows of Linux used by ping command to send just 1 ping
        ...
        # Define the ping command used for one ping " -n 1 -w 1000" or " -c 1 -w 1" (Windows and Linux versions are different)
    ...
    def write_file(self, file_type = 0, filename = "hosts.yaml"):
        ...
        # Check if a filenmae is really given (protection against calling the procedure without a correct name)
    ...
    def run(self):
        ...
        if self.network.num_addresses != 1:
            ...
            for host in self.network.hosts():
                ...
                # my_tasks is a list with coroutine tasks. It gets 2 parameters: one with the ping command
                # and the other one with the ip address of the target
        ...
        # if Windows is in use then these commands are needed otherwise "asyncio.create_subprocess_shell" will fail
...
if __name__ == '__main__':
    ...
    # - filename_to_save (string, optional): the name of the file with the result of the ping (default "None" means "hosts.yaml" will be used)
    #
    file_to_save = 0
